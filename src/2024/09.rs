...
/// Fills the free space. Moves file blocks one at a time starting from the highest file
...
/// Fills the free space without fragmentation. Moves whole files starting from the highest file
...
/// ID number it contains. The leftmost block is at position `0`. If a block
/// contains free space it is skipped.
///
/// # Arguments:
    ...
    disk.into_iter()
        ...
        .filter_map(|(i, digit)| {
            if digit == -1 {
                ...
                u64::try_from(i.checked_mul(digit.try_into().ok()?)?).ok()
            }
        })
        .sum()
}
...
fn parse_disk(input: &str) -> Vec<i32> {
    input
        ...
        .flat_map(|(file_id, size)| {
            ...
            iter::repeat_n(block_value, size)
        })
        .collect()
}
